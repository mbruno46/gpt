...
#include "WilsonExpCloverFermion.h"

template<typename vCoeff_t>
cgpt_fermion_operator_base* cgpt_create_wilson_exp_clover(PyObject* args) {
  ...
  auto f = new WilsonExpCloverFermion<WI>(U,*grid,*grid_rb,mass,csw_r,csw_t,isw,wac,wp);

  return new cgpt_fermion_operator<WilsonExpCloverFermion<WI>>(f);
}
